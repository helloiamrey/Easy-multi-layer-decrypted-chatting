...
def encrypt_data(data, pri_key, pub_key):
    """
    加密函数
    :param data: 要加密的字符串数据
    :param pri_key: 十六进制字符串格式的私钥
    :param pub_key: 十六进制字符串格式的公钥（已去掉04前缀）
    :return: 加密后的十六进制字符串
    """
...
def decrypt_data(cipher, pri_key, pub_key):
    """
    解密函数
    :param cipher: 要解密的十六进制字符串
    :param pri_key: 十六进制字符串格式的私钥
    :param pub_key: 十六进制字符串格式的公钥（已去掉04前缀）
    :return: 解密后的原始字符串
    """
...
def generate_keypair():
    #生成SM2密钥对
    #:return: (私钥, 公钥)
    secretint = int.from_bytes(os.urandom(32), 'big') % SM2Key.sm2p256v1.N
    e = SM2Util.GenKeyPair(secretint)
    prikey = e[0]  # 私钥（十六进制字符串）
    pubkey_without_prefix = e[1][2:]  # 去掉前缀的公钥
    return (prikey, pubkey_without_prefix)
...
if __name__ == '__main__':
    ...
    # 加密
    cipher = encrypt_data("secretTest", keypair[0], keypair[1])

    # 解密
    decrypted = decrypt_data(cipher, keypair[0], keypair[1])
